...
namespace TIS_3dAntiCollision.Core
{
    public class PLC
    {
        ...
        public byte[] ReadBytes(DataType DataType, int DB, int StartByteAdr, int count)
        {
            ...
            try
            {
                ...
                switch (DataType)
                {
                    case DataType.Timer:
                    case DataType.Counter:
                        package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)(StartByteAdr)));
                        break;
                    default:
                        package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)((StartByteAdr) * 8)));
                        break;
                }
        ...
        public object Read(DataType DataType, int DB, int StartByteAdr, VarType VarType, int VarCount)
        {
            ...
            switch (VarType)
            {
                ...
                case VarType.Word:
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.Word.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.Word.ToArray(bytes);
                case VarType.Int:
                    cntBytes = VarCount * 2;
                    bytes = ReadBytes(DataType, DB, StartByteAdr, cntBytes);
                    if (bytes == null) return null;
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.Int.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.Int.ToArray(bytes);
                case VarType.DWord:
                    cntBytes = VarCount * 4;
                    bytes = ReadBytes(DataType, DB, StartByteAdr, cntBytes);
                    if (bytes == null) return null;
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.DWord.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.DWord.ToArray(bytes);
                case VarType.DInt:
                    cntBytes = VarCount * 4;
                    bytes = ReadBytes(DataType, DB, StartByteAdr, cntBytes);
                    if (bytes == null) return null;
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.DInt.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.DInt.ToArray(bytes);
                case VarType.Real:
                    cntBytes = VarCount * 4;
                    bytes = ReadBytes(DataType, DB, StartByteAdr, cntBytes);
                    if (bytes == null) return null;
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.Double.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.Double.ToArray(bytes);
                case VarType.String:
                    cntBytes = VarCount;
                    bytes = ReadBytes(DataType, DB, StartByteAdr, cntBytes);
                    if (bytes == null) return null;

                    return TIS_3dAntiCollision.Core.PlcTypes.String.FromByteArray(bytes);
                case VarType.Timer:
                    cntBytes = VarCount * 2;
                    bytes = ReadBytes(DataType, DB, StartByteAdr, cntBytes);
                    if (bytes == null) return null;
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.Timer.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.Timer.ToArray(bytes);
                case VarType.Counter:
                    ...
                    if (VarCount == 1)
                        return TIS_3dAntiCollision.Core.PlcTypes.Counter.FromByteArray(bytes);
                    else
                        return TIS_3dAntiCollision.Core.PlcTypes.Counter.ToArray(bytes);
                default:
                    return null;
            }
            //return null;
        ...
        {
            double numBytes = TIS_3dAntiCollision.Core.PlcTypes.Struct.GetStructSize(structType);
            // now read the package
            byte[] bytes = (byte[])Read(DataType.DataBlock, DB, 0, VarType.Byte, (int)numBytes);
            // and decode it
            return TIS_3dAntiCollision.Core.PlcTypes.Struct.FromBytes(structType, bytes);
        }
        #endregion
        ...
        public ErrorCode WriteBytes(DataType DataType, int DB, int StartByteAdr, byte[] value)
        {
            ...
            try
            {
                ...
                TIS_3dAntiCollision.Core.PlcTypes.ByteArray package = new TIS_3dAntiCollision.Core.PlcTypes.ByteArray(packageSize);

                package.Add(new byte[] { 3, 0, 0 });
                package.Add((byte)packageSize);
                package.Add(new byte[] { 2, 0xf0, 0x80, 0x32, 1, 0, 0 });
                package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)(varCount - 1)));
                package.Add(new byte[] { 0, 0x0e });
                package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)(varCount + 4)));
                package.Add(new byte[] { 0x05, 0x01, 0x12, 0x0a, 0x10, 0x02 });
                package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)varCount));
                package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)(DB)));
                package.Add((byte)DataType);
                package.Add((byte)0);
                package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)(StartByteAdr * 8)));
                package.Add(new byte[] { 0, 4 });
                package.Add(TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((ushort)(varCount * 8)));
        ...
        public object Write(DataType DataType, int DB, int StartByteAdr, object value)
        {
            ...
            {
                case "Byte":
                    package = TIS_3dAntiCollision.Core.PlcTypes.Byte.ToByteArray((byte)value);
                    break;
                case "Int16":
                    package = TIS_3dAntiCollision.Core.PlcTypes.Int.ToByteArray((Int16)value);
                    break;
                case "UInt16":
                    package = TIS_3dAntiCollision.Core.PlcTypes.Word.ToByteArray((UInt16)value);
                    break;
                case "Int32":
                    package = TIS_3dAntiCollision.Core.PlcTypes.DInt.ToByteArray((Int32)value);
                    break;
                case "UInt32":
                    package = TIS_3dAntiCollision.Core.PlcTypes.DWord.ToByteArray((UInt32)value);
                    break;
                case "Double":
                    package = TIS_3dAntiCollision.Core.PlcTypes.Double.ToByteArray((System.Double)value);
                    break;
                case "Byte[]":
                    package = (byte[])value;
                    break;
                ...
                case "UInt32[]":
                    ...
                    break;
                case "Double[]":
                    package = TIS_3dAntiCollision.Core.PlcTypes.Double.ToByteArray((double[])value);
                    break;
                case "String":
                    package = TIS_3dAntiCollision.Core.PlcTypes.String.ToByteArray(value as string);
                    break;
                default:
        ...
        public ErrorCode WriteStruct(object structValue, int DB)
        {
            try
            {
                byte[] bytes = TIS_3dAntiCollision.Core.PlcTypes.Struct.ToBytes(structValue);
